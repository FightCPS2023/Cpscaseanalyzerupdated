...
import {
  LegalDisclaimerPages as LegalDisclaimer,
  FooterDisclaimer,
  AIContentDisclaimer,
} from "./components/LegalDisclaimerPages";
...
export default function App() {
  ...
  const [showLandingPage, setShowLandingPage] = useState(() => {
    ...
    const hasSeenLanding = localStorage.getItem(
      "cpsHasSeenLanding",
    );
  ...
  const [caseToEdit, setCaseToEdit] = useState<
    CaseData | undefined
  >(undefined);
  const [caseManagerMode, setCaseManagerMode] = useState<
    "create" | "edit"
  >("create");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (GA_MEASUREMENT_ID) {
        ...
        if (isDev()) {
          console.info(
            "ℹ️ Google Analytics not configured. To enable tracking, add VITE_GA_MEASUREMENT_ID to your .env file.",
          );
        }
      }
    } catch (error) {
      if (isDev()) {
        console.warn(
          "⚠️ Could not initialize Google Analytics:",
          error,
        );
      }
    }
  ...
  const detectUserLocation = async () => {
    try {
      ...
      if (data.region) {
        ...
        if (!savedState) {
          toast.info(
            `Location detected: ${data.region}. Set your state for customized legal information.`,
          );
        }
      }
    } catch (error) {
  ...
  const handleStateChange = (state: string) => {
    ...
    toast.success(
      `State set to ${state}. Information customized for your jurisdiction.`,
    );
  };
        ...
        if (data.cases && data.cases.length > 0) {
          ...
          if (data.activeCaseId) {
            const active = data.cases.find(
              (c: CaseData) => c.id === data.activeCaseId,
            );
            if (active) {
  ...
  const syncFromCloud = async (accessToken: string) => {
    try {
      ...
      if (
        ...
        setCaseDetails(
          response.data.caseDetails || caseDetails,
        );
        ...
        if (
          ...
          if (response.data.activeCaseId) {
            const active = response.data.cases.find(
              (c: CaseData) =>
                c.id === response.data.activeCaseId,
            );
            if (active) {
              setActiveCase(active);
            }
  ...
  const handleAuthSuccess = (
    ...
    api
      ...
      .then(() => {
        toast.success(
          "Your local data has been backed up to the cloud!",
        );
      })
      .catch((error) => {
  ...
  const handleLogout = () => {
    ...
    setUseLocalStorage(true);
    toast.success(
      "Logged out - continuing in local storage mode",
    );
  };
  ...
  const editTimelineEvent = (
    id: string,
    updatedEvent: Omit<TimelineEvent, "id">,
  ) => {
    setTimelineEvents((prev) =>
      prev.map((event) =>
  ...
  const handleViolationsDetected = (
    ...
    if (detectedViolations.length > 0) {
      ...
      detectedViolations.forEach((violation) => {
        if (
          violations[violation as keyof typeof violations] ===
          false
        ) {
      ...
      if (newViolationsFound) {
        ...
        toast.success(
          `Document analyzed! ${detectedViolations.length} violation(s) identified and automatically added.`,
        );
      }
    }
  };

  const handleTimelineEventsDetected = (
    events: Array<{
      date: string;
      title: string;
      description: string;
    ...
    if (events.length > 0) {
      ...
      toast.success(
        `${events.length} timeline event(s) extracted from document.`,
      );
    }
  };

  const handleCaseInfoDetected = (info: {
    caseNumber?: string;
    dates?: string[];
    names?: string[];
    locations?: string[];
    ...
    if (
      ...
      const countyMatch = info.locations.find((loc) =>
        loc.includes("County"),
      );
      if (countyMatch) {
  ...
  const handleGetStarted = (
    selectedUserType: "parent" | "attorney",
  ) => {
    setUserType(selectedUserType);
    setShowLandingPage(false);
    localStorage.setItem("cpsHasSeenLanding", "true");
    trackCPSEvent.userAction(
  ...
  const handleOnboardingComplete = (
    ...
    toast.success(
      `Welcome! Your case "${caseData.caseName}" has been created.`,
    );
  ...
  const handleSaveCase = (caseData: CaseData) => {
    if (caseManagerMode === "create") {
      ...
      setActiveCase(caseData);
      toast.success(
        `Case "${caseData.caseName}" created successfully!`,
      );
    } else {
      setCases((prev) =>
      ...
      toast.success(
        `Case "${caseData.caseName}" updated successfully!`,
      );
    }
  };
  ...
  const handleDeleteCase = (caseId: string) => {
    ...
    if (activeCase?.id === caseId) {
      ...
      setActiveCase(
        remainingCases.length > 0 ? remainingCases[0] : null,
      );
    }

    toast.success(
      `Case "${caseToDelete?.caseName}" deleted successfully.`,
    );
  };
  ...
  const handleSelectCase = (caseId: string) => {
    ...
    if (selectedCase) {
      ...
      toast.success(
        `Switched to case: ${selectedCase.caseName}`,
      );
    }
  };
  ...
  if (showLandingPage) {
    return (
      <LandingPageRouter onGetStarted={handleGetStarted} />
    );
  }
  ...
  if (!auth.accessToken) {
    return (
      <SubscriptionProvider>
        <AuthForm
          onAuth={(userId, accessToken, tier) => {
  ...
  const violationCount =
    Object.values(violations).filter(Boolean).length;
    ...
    <ErrorBoundary>
      <AccessibilityProvider>
        <SubscriptionProvider>
          ...
          <div
            className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:from-gray-900 dark:via-black dark:to-gray-900"
            lang="en"
          >
            <header
              className="sticky top-0 z-50 border-b bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-sm"
              role="banner"
            >
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-3">
                    ...
                    {auth.accessToken && (
                      <Button
                        ...
                        className="h-8 sm:h-10 px-2 sm:px-4 border-2 hover:bg-gray-100 dark:hover:bg-gray-800"
                      >
            ...
            <main
              id="main-content"
              className="container mx-auto px-2 sm:px-4 py-4 sm:py-6 max-w-7xl"
              role="main"
            >
              <Alert
                className="mb-4 sm:mb-6 border-l-4 border-l-red-600 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-950/30 dark:to-orange-950/30 shadow-sm w-full"
                role="alert"
                aria-live="polite"
              >
                <div className="flex items-start sm:items-center gap-2 sm:gap-3">
                  <div
                    className="w-8 h-8 sm:w-10 sm:h-10 bg-red-600 rounded-lg flex items-center justify-center flex-shrink-0"
                    aria-hidden="true"
                  >
                    <Shield className="h-4 w-4 sm:h-5 sm:w-5 text-white" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <AlertTitle className="text-xs sm:text-sm font-bold text-gray-900 dark:text-white mb-1 text-justify">
                      ⚖️ Educational Tool - Not Legal Advice
                    </AlertTitle>
                    <AlertDescription className="text-xs text-gray-700 dark:text-gray-300 text-justify">
                      <strong>Important:</strong> This tool
                      helps you identify potential issues in
                      your CPS case for educational purposes
                      only.
                      <strong>
                        {" "}
                        This does not constitute legal advice.
                      </strong>{" "}
                      Always consult with a qualified family law
                      attorney licensed in your state for legal
                      advice specific to your situation. Laws
                      vary by jurisdiction and individual
                      circumstances.
                      {useLocalStorage &&
                        " Currently using local storage - sign in for cloud backup."}
                    </AlertDescription>
                  </div>
                </div>
              </Alert>
              ...
              <Card
                className="mb-4 sm:mb-6 p-4 sm:p-6 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-950/40 dark:to-orange-950/40 border-2 border-primary/30 dark:border-primary/50 shadow-lg"
                role="region"
                aria-label="State Selection"
              >
                <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                  <div
                    className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-primary to-red-700 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg"
                    aria-hidden="true"
                  >
                    <MapPin className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                  </div>
                  <div className="flex-1 min-w-0 w-full">
                    <h2 className="mb-2 text-base sm:text-lg font-bold text-red-900 dark:text-red-100">
                      Set Your State for Customized Legal
                      Information
                    </h2>
                    <p className="text-xs sm:text-sm text-red-800 dark:text-red-200 mb-3 sm:mb-4 leading-relaxed">
                      <strong>Important:</strong> CPS laws and
                      procedures vary significantly by state.
                      Select your state to receive
                      jurisdiction-specific guidance, case law,
                      and defense strategies. This ensures the
                      information you receive is relevant to
                      your legal jurisdiction.
                      {detectedLocation &&
                        !userState &&
                        ` We detected you may be in ${detectedLocation}.`}
                    </p>
              ...
              {/* Case Selector */}
              <Card
                className="mb-4 sm:mb-6 p-4 sm:p-5 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-950/40 dark:to-orange-950/40 border-2 border-primary/30 dark:border-primary/50 shadow-lg"
                role="region"
              ...
              >
                <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-xl border-2 border-gray-200 dark:border-gray-700 p-2 sm:p-3 mb-4 sm:mb-6 overflow-x-auto">
                  <TabsList className="inline-flex lg:grid lg:grid-cols-8 xl:grid-cols-15 gap-1.5 sm:gap-2 bg-transparent p-0 h-auto min-w-max lg:min-w-0 lg:w-full">
                    ...
                    >
                      ...
                      <span>Binder</span>
                      <Crown
                        className="w-3 h-3 text-amber-500 ml-1"
                ...
                <WelcomeTour
                  onComplete={() => {
                    toast.success(
                      "Welcome! Start by uploading a document or exploring your rights.",
                    );
                  }}
                />
                ...
                <TabsContent value="policy">
                  {!userState && (
                    <ActionWarning
                      ...
                      onAction={() => {
                        toast.info(
                          "Select your state from the dropdown above to access state-specific CPS policies",
                        );
                        window.scrollTo({
                          top: 0,
                          behavior: "smooth",
                  ...
                  <CPSPolicyEngine
                    ...
                    onViolationFound={(
                      ...
                      toast.info(
                        `Violation reference added: ${reference}`,
                      );
                    }}
                  />
                </TabsContent>
                ...
                <TabsContent value="documents">
                  ...
                  <CaseDocuments
                    ...
                    onViolationsDetected={
                      handleViolationsDetected
                    }
                    onTimelineEventsDetected={
                      handleTimelineEventsDetected
                    }
                ...
                <TabsContent value="timeline">
                  ...
                  <Card className="p-6">
                    <CaseTimeline
                      ...
                      onDeleteEvent={removeTimelineEvent}
                ...
                <TabsContent value="violations">
                  ...
                  {activeCase && !userState && (
                    <ActionWarning
                      ...
                      onAction={() => {
                        toast.info(
                          "Select your state from the dropdown above",
                        );
                        window.scrollTo({
                          top: 0,
                          behavior: "smooth",
                ...
                <TabsContent value="generator">
                  ...
                  {activeCase && !userState && (
                    <ActionWarning
                      ...
                      onAction={() => {
                        toast.info(
                          "Select your state from the dropdown above to generate state-specific documents",
                        );
                        window.scrollTo({
                          top: 0,
                          behavior: "smooth",
